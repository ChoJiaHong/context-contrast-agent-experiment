...
def score_output(output, task):
    ...
    removal_states=[state for state in output.reasoning_trace if "incidental_removed" in state]
    removal_rate=sum(bool(state["incidental_removed"]) for state in removal_states)/len(removal_states) if removal_states else 0.0
    return {"context_precision":differences["precision"],"context_recall":differences["recall"],"context_f1":differences["f1"],"constraint_recall":set_scores(output.constraints,gt.changed_constraints,aliases)["recall"],"resource_recall":set_scores(output.resources,gt.available_resources,aliases)["recall"],"assumption_recall":set_scores(output.assumption_changes,gt.changed_assumptions,aliases)["recall"],"cost_structure_recall":cost["recall"],"false_context_difference_rate":1-differences["precision"],"essential_context_precision":essential["precision"],"essential_context_recall":essential["recall"],"incidental_context_removal_rate":removal_rate,"objective_success":objective,"strategy_validity":output.strategy_family in gt.valid_strategy_families,"over_reframing":over}
