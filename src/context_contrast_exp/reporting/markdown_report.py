...
def generate_report(results_path: str, out: str) -> None:
    ...
    lines += [", ".join(f"{k}: {v}" for k,v in sorted(counts.items())),"","## Aggregate results table","| Method | Objective mean | 95% bootstrap CI | Context F1 | Calls | Tokens |","|---|---:|---:|---:|---:|---:|"]
    for method, rs in grouped.items():
        ...
        lines.append(f"| {method} | {obj['mean']:.3f} | [{obj['ci_low']:.3f}, {obj['ci_high']:.3f}] | {f1['mean']:.3f} | {sum(x['calls'] for x in rs)/len(rs):.1f} | {sum(x['input_tokens']+x['output_tokens'] for x in rs)/len(rs):.1f} |")
    ...
    sections=[("Downward-loop ablation","Compare bidirectional_loop with upward_loop, and downward_loop with single pass. Mock output is not an ablation result."),("Upward-loop ablation","Compare bidirectional_loop with downward_loop; essential-context precision/recall are independently reported."),("Full vs single-pass comparison","Compare objective success under equal budgets; richer text alone is not success."),("Negative-control / over-reframing analysis",f"Mean over-reframing: {sum(float(r['metrics']['over_reframing']) for r in negative)/max(1,len(negative)):.3f}."),("Efficiency analysis",f"Total calls: {sum(r['calls'] for r in records)}; total tokens: {sum(r['input_tokens']+r['output_tokens'] for r in records)}; approximate cost: {sum(r['approximate_cost'] for r in records):.6f}."),("Failure cases","Review records with objective success false and adjudicate semantic mismatches."),("Threats to validity","Six hand-built tasks are underpowered; alias coverage, model dependence, mock leakage, and evaluator validity limit inference."),("Go / Weak / No-Go decision","Mock run: **No decision**. For real runs apply the predefined criteria without reinterpretation."),("Raw artifact paths and reproduction command",f"- Results: {results_path}\n- Reproduce: python -m context_contrast_exp.cli report --results {results_path} --out {out}")]
