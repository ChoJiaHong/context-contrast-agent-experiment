...
from .schemas import LLMResponse, MethodOutput
...
class MockLLMClient:
    ...
    def generate(self, *, system: str, user: str, seed: int | None = None) -> LLMResponse:
        started=time.perf_counter(); payload=json.loads(user); task=payload["task"]; method=payload["method"]
        ...
        match=next((value for needle,value in rules if needle in text),("",[],[],[],[],"conventional direct solution")); difference,constraints,resources,assumptions,costs,strategy=match
        ...
        response=self.client.chat.completions.create(
            ...
            response_format={
                ...
                "json_schema":{
                    "name":"method_output",
                    "strict":True,
                    "schema":MethodOutput.model_json_schema(),
                },
            },
